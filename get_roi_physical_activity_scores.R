get_roi_physical_activity_scores <- function(project="nih-nci-dceg-connect-stg-5519",
  ...
  sql <- glue::glue("SELECT Connect_ID
                     FROM `{project}.ReturnOfInformation.physical_activity`
                     WHERE Connect_ID IS NOT NULL")
  result <- DBI::dbGetQuery(con, sql)

  if (include_only_updates & length(result$Connect_ID) > 0 ) {
    connect_ids_to_exclude <- paste(shQuote(result$Connect_ID, type = "cmd"), collapse = ", \n")
  } else {
  ...
  physical_activity_ROI_final <- physical_activity_ROI_BC %>%
    ...
    mutate(
      ...
      strengthening_binary = case_when(
        ...
        strengthening_binary == "No" ~ FALSE,       # "No" becomes FALSE
        TRUE ~ NA                                   # Handles unexpected values
      ),
      true_missing = case_when(
        true_missing == 1 ~ TRUE,                   # 1 becomes TRUE
        true_missing == 0 ~ FALSE,                  # 0 becomes FALSE
        TRUE ~ NA                                   # Handles unexpected values
      )
    )
